...
<script type="text/javascript">
  ...
  <body>
  <form method="post" action="grantSubmit.do">
			<table width="100%">
				<tr>
					<td>
				...
				<tr>
					<td>
						...
						<span id="salarySum_sum">${sg.salaryPaidSum }</span>
							</td>
						<td align="right">登记人：<input type="text" name="register" value="admin" style="width:80px;background-color:#D4F8D4;">登记时间：
							<span id="datetime">
    						<script>
				 ...
				 <c:forEach items="${sgdlist}" var="s" varStatus="vs">
				  <input type="hidden" name="grantDetails[${vs.index }].salaryGrantId" value="${s.humanId }">
				 	...
				 	<tr class="TD_STYLE2">
						...
						<td>
							<input type="text" name="bounsSum"  id="bounsSum${vs.count }"  onkeyup="onKeyPress('${vs.count}')" value="0.0" class="INPUT_STYLE2" />
						</td>
						<td>
							<input type="text" name="saleSum"   id="saleSum${vs.count }"  onkeyup="onKeyPress('${vs.count }')" value="0.0" class="INPUT_STYLE2"/>
						</td>
						<td>
							<input type="text" name="deductSum"   id="deductSum${vs.count }" onkeyup="onKeyPress('${vs.count }')" value="0.0" class="INPUT_STYLE2"/>
						</td>
						<td>
							<input type="text" name="salaryPaidSum" readonly="readonly"  id="salaryPaidSum${vs.count }" value="${s.salaryStandardSum } "/>
						</td>
					</tr>	
					</c:forEach>
